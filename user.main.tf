resource "googleworkspace_user" "primary_email" {
  ...
  dynamic "custom_schemas" {
    for_each = { for custom_schema in var.custom_schemas : custom_schema.schema_name => custom_schema if custom_schema.schema_name != null }
    content {
  ...
  dynamic "relations" {
    for_each = { for index, relation in var.relations : index => relation if relation.type != null }
    content {
  ...
  dynamic "ssh_public_keys" {
    for_each = { for index, ssh_public_key in var.ssh_public_keys : index => ssh_public_key if ssh_public_key.key != null }
    content {
